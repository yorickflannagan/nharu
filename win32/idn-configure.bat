...
SET _CVARS_=-DWIN32 -D_WIN32 -DIDNA_EXPORTS -DHAVE_CONFIG_H -D_CRT_SECURE_NO_DEPRECATE -D_CRT_NONSTDC_NO_DEPRECATE -D_MBCS -D_USRDLL -D_WINDLL
SET _CDEBUG_=/GL /analyze-
SET _CFLAGS_=/W3 /Zc:wchar_t /Zi /Gm- /O2 /WX- /Gd /Oy- /nologo
SET _LFLAGS_=/MANIFEST /LTCG /NXCOMPAT /DYNAMICBASE /DLL /MACHINE:X86 /SAFESEH /MANIFESTUAC:"level='asInvoker' uiAccess='false'" /NOLOGO /TLBID:1
...
FOR /F %%a IN ('dirname %_PACKAGE_%') DO SET _PACKAGE_=%%a
REM NMAKE does not support Windows SDK %INCLUDE% variable with a list of directories;
REM so we must split them to compiler
...
ECHO --cvars: compiler variables definitions by -D. Default value: -DWIN32
